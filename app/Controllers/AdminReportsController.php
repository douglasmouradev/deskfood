...
final class AdminReportsController extends Controller
{
    ...
    {
        $pdo = Database::pdo();
        $today = \App\Helpers\DbDate::todayWhere();
        $ordersToday = (int) $pdo->query("SELECT COUNT(*) FROM orders WHERE {$today} AND deleted_at IS NULL")->fetchColumn();
        $revenueToday = (float) $pdo->query("SELECT COALESCE(SUM(total),0) FROM orders WHERE {$today} AND payment_status IN ('pago','confirmado_entrega') AND deleted_at IS NULL")->fetchColumn();
        $byDay = $pdo->query(
